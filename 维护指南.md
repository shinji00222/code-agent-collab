...
检索来源在 `.agent-workbench/config.json` 的 `mainVaultPath`，或用环境变量 `AGENT_WORKBENCH_MAIN_VAULT` 覆盖；
**默认是项目自有知识库 `dev-vault/project-vault`**，不配置就不会读任何外部知识库。

### 修改知识库读写目标（隔离相关）

读取来源 = `mainVaultPath` / `AGENT_WORKBENCH_MAIN_VAULT`；
