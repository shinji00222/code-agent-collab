...
2. 运行 `review`：先本地扫描敏感信息（API 密钥、密码/令牌、手机号、邮箱），再让 AI 判断价值、重复和写入位置。
3. 安全候选自动写入主知识库对应位置，状态变为"已入库"；敏感或 AI 不建议的候选状态变为"待人工处理"。
4. 待人工处理的候选由你决定：`confirm` 强制入库（仍会拦截敏感信息）或 `discard` 废弃。
