...
一个本地优先的命令行工具：你给它一个任务，它按顺序或按主控方案让多个 Agent 协作（拆任务 → 查知识 → 出计划 → 写代码草稿 → 检查 → 复盘），结果先进 `dev-vault` 待确认区，不直接改正式源码、不写主知识库。
...
│   ├── agents/              # Agent 角色（coordinator、knowledge、planner、coder、reviewer、orchestrator、validator、reflector）
...
│   └── main-vault-sandbox/  # 知识写入沙箱：confirm 的默认落点，与真实知识库隔离
...
生成上下文包 logs/context-packs/<任务ID>.md（任务目标、项目文档、知识库边界、Git 状态）
      ↓
默认流水线中的 Agent 按顺序执行，每个产出 AgentResult：
...
开始维护前先确认这几件事，避免在错误目录或错误版本上工作：

```powershell
cd "$env:USERPROFILE\Desktop\AI工作台知识库\01-项目\project 多Agent代码协作助手"
...
- 真实项目根是 `C:\Users\<你的用户名>\Desktop\AI工作台知识库\01-项目\project 多Agent代码协作助手`，不要把 `Documents\ChatGPT\agent协作` 当源码根。
- `git status --short --branch` 应能显示 `main...origin/main` 和 ahead/behind；若显示没有 upstream，先执行 `git branch --set-upstream-to=origin/main main`。
- 本地 ahead 代表还有提交没有推送到 GitHub；这不是错误，但发布、协作或对外说明前必须先讲清楚哪些提交还只在本机。
- 修改 Web UI、Provider、编排、发布脚本前，测试默认使用 mock：`$env:AGENT_WORKBENCH_PROVIDER="mock"`。
...
python -m code_agent_collab.cli plans          # 查看待批准/已执行的主控方案
...
python -m code_agent_collab.desktop             # 启动本地桌面窗口
python -m code_agent_collab.webui               # 启动备用网页终端（127.0.0.1:8080）
...
5. **写入目标是 `mainVaultWritePath`，默认是项目内沙箱 `dev-vault/main-vault-sandbox`，不是真实主知识库。** 想真的写进主知识库，必须显式配置这一项，细节见 `dev-vault/main-vault-sandbox/README.md`。
...
写入目标在 `config.json` 的 `mainVaultWritePath`，或用环境变量 `AGENT_WORKBENCH_MAIN_VAULT_WRITE` 覆盖：

- 不配置 → 默认项目内 `dev-vault/main-vault-sandbox`，真实知识库零写入（安全默认）。
- 配置成主知识库路径 → `confirm` 会真的写进主知识库，**这是唯一会碰真实知识库的开关**。
- 默认值的来源是 `config.py` 的 `default_write_vault_path()`；改动它等于改变全项目的安全默认，属于高影响改动。
...
```json
{
  ...
  "mainVaultWritePath": "项目的 dev-vault\\main-vault-sandbox 路径"
}
```
