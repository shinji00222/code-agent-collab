...
- 下一步 UI 目标：在 Web 终端进度树里点击某个 Agent，可以看到它的 `owned_paths`、当前状态、WorkerRun 尝试次数、输出路径、失败原因、阻塞信息和下一步。
- 实现顺序建议：先把 blackboard 摘要和 WorkerRun 尝试次数整合成详情数据，再做最小 Agent 详情面板；不要重做整个 Web UI。
