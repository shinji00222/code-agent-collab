...
- 下一步优先做共享黑板基础版：每个 Agent 写入自己的职责、认领路径、输出、阻塞和备注，作为 `/api/progress` 的可读详情数据。
- 后续 UI 目标：在 Web 终端进度树里点击某个 Agent，可以看到它的 `owned_paths`、当前状态、WorkerRun 尝试次数、输出路径、失败原因、阻塞信息和下一步。
- 实现顺序建议：先定 Blackboard 数据结构并接入 Agent 写入，再让 `/api/progress` 暴露详情，最后做最小 Agent 详情面板；不要先重做 UI，避免数据结构变化导致返工。
