...
- `apply-draft`：解析 Coder/Integrator 草稿并预览 diff（dry-run，不写文件）；草稿必须满足“修改文件清单”和“建议代码”路径一致，并通过评审闸门；加 `--apply` 后才应用改动 → 自动跑测试 → 测试通过自动本地 commit（失败自动回滚）。只允许改 `src/`、`tests/` 下文本文件。
