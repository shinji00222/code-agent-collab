...
当前版本增加了统一的 AI Provider 接口，`PlannerAgent` 已经通过这个接口获取计划结果，但默认仍使用本地 `mock` Provider，不会联网，也不需要 API Key。

以后接入真实 AI 时，Agent 只调用统一接口，不直接管理密钥。密钥只从环境变量读取，不写入项目文件、日志或知识库。

目前提供了两个预设：

- `deepseek`：接口地址使用 `https://api.deepseek.com`，默认模型使用 `deepseek-v4-flash`，密钥环境变量使用 `DEEPSEEK_API_KEY`。
